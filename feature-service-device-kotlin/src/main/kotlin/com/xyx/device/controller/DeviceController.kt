...
class DeviceController(private val deviceRepository: DeviceRepository) {
    ...
    fun queryAll(dto: DeviceSearchDto) =
        returnSuccess(deviceRepository.query(dto, emptyArray(), arrayOf("name")).map { DeviceListVo.vo(it) })
    ...
    fun save(dto: DeviceSaveDto): Return {
        ...
        //deviceRepository.save(Device(dto.uid,null))
    ...
    fun update(dto: DeviceUpdateDto): Return {
        ...
        if (entityOptional.isPresent) {
            ...
            deviceRepository.save(entityOptional.get().apply { uid = dto.uid })
            return returnSuccess()
        }
